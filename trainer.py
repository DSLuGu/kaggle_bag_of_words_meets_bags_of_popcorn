    ...
    def attach(train_engine, valid_engine, verbose=VERBOSE_BATCH_WISE):
        ...
        if verbose >= VERBOSE_EPOCH_WISE:
            ...
            def print_train_logs(engine):
                print(
                    "[Train] Epoch {} - |param|={:.2e}, |g_param|={:.2e}, loss={:.4e}, accuracy={:.4e}".format(
                        engine.state.epoch, 
        ...
        if verbose >= VERBOSE_EPOCH_WISE:
            ...
            def print_valid_logs(engine):
                print(
                    "[Validation] - loss={:.4e}, accuracy={:.4e}, best_loss={:.4e}".format(
                        engine.state.metrics['loss'], 
                        engine.state.metrics['accuracy'], 
                        engine.best_loss, 
                    )
...
class Trainer:
    ...
    def train(self, model, criterion, optimizer, train_loader, valid_loader):
        ...
        train_engine = MyEngine(MyEngine.train, model, criterion, optimizer, self.config)
        valid_engine = MyEngine(MyEngine.validate, model, criterion, optimizer, self.config)
        ...
        train_engine.add_event_handler(
            Events.EPOCH_COMPLETED, run_validation, valid_engine, valid_loader, 
        )
        valid_engine.add_event_handler(
            Events.EPOCH_COMPLETED, MyEngine.check_best, 
        )
        # valid_engine.add_event_handler(
        #     Events.EPOCH_COMPLETED, MyEngine.save_model, train_engine, self.config, 
        # )
